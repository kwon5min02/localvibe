...
const PAGE_INFO = {
  gallery: {
    title: '갤러리',
    subtitle: '가고 싶은 곳을 검색하거나, 새로운 로컬 스팟을 발견해보세요.',
  },
  planner: {
    title: '여행 플래너',
    subtitle: '챗봇과 함께 나만의 여행 일정을 만들어보세요.',
  },
  mypage: {
    title: '마이페이지',
...
function pickFeedItems(items, size = FEED_SIZE) {
  ...
  for (const item of shuffled) {
    const nk = normalizeTextKey(item?.name),
      ik = normalizeImageKey(item?.imageUrl);
...
export default function App() {
  ...
  const [displayedRegions, setDisplayedRegions] = useState(
    () => readPersistedGalleryRegions() ?? DEFAULT_REGIONS_NORMALIZED,
  );
  ...
  const [myTrips, setMyTrips] = useState(() => {
    try {
      const p = JSON.parse(localStorage.getItem('lv_my_trips') || '[]');
      return Array.isArray(p) ? p : [];
    } catch {
      return [];
    }
    ...
    fetch(`${API_BASE_URL}/api/regions`)
      ...
      .then(data => {
        ...
        if (
          ...
          setDisplayedRegions(pickFeedItems(normalized));
      })
      .catch(() => {
        if (
          !galleryVectorSearchActiveRef.current &&
    ...
    (async () => {
      try {
        ...
        if (artRes.ok) {
          ...
          if (!cancelled)
            setModalArticle({ title: a.title || '', content: a.content || '' });
    ...
    tripId => {
      ...
      setMyTrips(prev => {
        ...
        const next = prev.map(t =>
          t.id === tripId ? { ...t, places: [...t.places, region] } : t,
        );
        localStorage.setItem('lv_my_trips', JSON.stringify(next));
        return next;
      });
                ...
                >
                  ...
                  <span
                    style={{
                      ...
                      transform: openRegions[r.id] ? 'rotate(180deg)' : 'none',
                    }}
                  >
                    ▼
                  </span>
                  ...
                  <div style={{ paddingLeft: 8 }}>
                    {r.children.map(city => (
                      <button
                        ...
                        onClick={() => {
                          handleGalleryVectorSearch(city);
